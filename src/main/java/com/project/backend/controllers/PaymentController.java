...
public class PaymentController {
        ...
        private final PaymentRepository paymentRepository; // For direct repository access if needed
        ...
        public ResponseEntity<ResponseObject> createPayment(
            ...
            ResponseObject response = paymentService.createPayment(student.getStudentId(), a4Count, a3Count, "MoMo", "pay with MoMo");
        ...
        @GetMapping("/student")
        @PreAuthorize("hasRole('STUDENT')")
        public ResponseEntity<ResponseObject> findPaymentByStudentToken(@RequestHeader("Authorization") String authorizationHeader) throws Exception {
            // Extract token from the Authorization header
            String token = authorizationHeader.startsWith("Bearer ") ? authorizationHeader.substring(7) : authorizationHeader;
